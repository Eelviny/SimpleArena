...
public class SignListeners implements Listener {
                                ...
                                if(p.hasPermission("simplearena.join.sign") || e.getPlayer().isOp()){
                                    if(arena.contains(p)){
                                        if(arena.isMaxed()){
                                            ...
                                            arena.sendMessage(Messages.get("simplearena.leave.normal").replace("%player%", p
                                                    .getName()));
                                ...
                                if(clazz != null){
                                    ...
                                    if(playersArena != null){
                                        if(playersArena.canChangeClass(p)){
                                            ...
                                            p.sendMessage(Messages.get("simplearena.class.join").replace("%Clazz%", clazz
                                                    .getName()));
                                        }else{
                                            if(playersArena.inProgress())
                                                p.sendMessage(Messages.get("simplearena.class.long").replace("%time%", String
                                                        .valueOf(playersArena.getkitTime())));
                                            else
    ...
    private String removeColorCodes(String s){
        ...
        for(int i = 0; i < sb.length(); i++){
            ...
            if(c == '&' || c == '§'){
                sb.deleteCharAt(i);
                if(sb.length() > i) sb.deleteCharAt(i);
            }
        }
        return sb.toString();
    }
