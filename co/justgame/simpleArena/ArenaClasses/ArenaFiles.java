    ...
    public static ArrayList<Arena> loadArenas(){
        ...
        for(File file: arenaFolder.listFiles()){
            try{
                ...
                Bukkit.getLogger().log(Level.SEVERE, ChatColor.RED
                        + "Failed to load Arenas! Reason: Missing team spawn point or default class!");
            }catch (Exception e){
                Bukkit.getLogger()
                        .log(Level.SEVERE, ChatColor.RED + "Failed to load Arenas! Reason: unparsable value in config!");
                e.printStackTrace();
            }
        }
        return arenas;
