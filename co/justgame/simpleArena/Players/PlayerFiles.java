...
public class PlayerFiles {
	...
	public synchronized static void savePlayerInven(final Player p){
		...
		writeFile(SimpleArena.getUUID(p), p.getHealth(), p.getFoodLevel(), p.getLevel(), 
				loc.getX(), loc.getY(), loc.getZ(), loc.getWorld().getName(), p.getActivePotionEffects(), 
				p.getInventory().getContents(), p.getInventory().getArmorContents());
	}
	
	private static void writeFile(final String UUID, final Double health, final int hunger, final int levels, final Double X, final Double Y, final Double Z
	...
	public synchronized static void loadPlayerInvenWithoutThreading(final Player p){
		 ...
		 setPlayer(p, config.getDouble("health"), config.getInt("hunger"), config.getInt("levels"),
				  config.getDouble("location.x"), config.getDouble("location.y"),config.getDouble("location.z"),
				 config.getString("location.world"), potionEffects, inven, armor);
	}
